...
- [09 — Anomaly Detection](../09-anomaly-detection/README.md) — early signals and detection limits
- [10 — Alert Correlation](../10-alert-correlation/README.md) — layered alert grouping
- [11 — Root Cause Analysis](../11-root-cause-analysis/README.md) — proximate vs systemic cause
- [13 — Remediation](../13-remediation-safety/README.md) — safety gate, blast radius, rollback
- [14 — Production](../14-production-engine/README.md) — chaos, DR, runbook, maturity
...
- [07 — Kafka](../07-kafka/README.md) — control plane / data plane separation in signal transport
- [12 — LLM Agent](../12-investigation-engine/README.md) — when LLMs help investigation and when they only add noise

## Next Reading

After this chapter, return to [14 — Production](../14-production-engine/README.md) to apply design-review checklists and game days to your AIOps platform; or expand your personal incident library per section 11.
...
2. **Audit + change stream for ops tools**
   - Every capacity remove / config push emits an event into [07 — Kafka](../07-kafka/README.md) topic `aiops-change-events`
   - Correlation engine ([10](../10-alert-correlation/README.md)) prioritizes changes in a 15–30 minute window
...
3. **Status / observability independence**
   ...
   - See [01 — Observability](../01-observability/README.md) and [14 — Production](../14-production-engine/README.md)
...
> [!NOTE]
> **AIOps-as-consumer**
> The AIOps platform in this handbook often uses S3-compatible storage for Loki/Tempo/Thanos. Re-read [04 — Loki](../04-loki/README.md), [05 — Tempo](../05-tempo/README.md), [14 — Production](../14-production-engine/README.md) with the question: “If S3-2017 class hits the telemetry bucket, does on-call still have eyes?”
...
3. **Dependency inventory**
   - Catalog “who depends on DynamoDB/regional endpoint” for correlation topology ([11 — RCA](../11-root-cause-analysis/README.md))

4. **Break-glass architecture** ([13 — Remediation](../13-remediation-safety/README.md), [14 — Production](../14-production-engine/README.md))
...
> Rule: multi-phase incidents need a **composite recovery score** (DNS + API + dependency fan-out + backlog). See [10 — Alert Correlation](../10-alert-correlation/README.md) for incident lifecycle states.
...
> [!TIP]
> **AIOps / SRE lesson**
> Dependency inventory must include **physical, identity, badge, DNS, BGP, out-of-band console**. An AIOps topology that only draws the service mesh is **incomplete**. When designing [11 — RCA](../11-root-cause-analysis/README.md), add node types: `network_fabric`, `identity`, `physical_access`, `dns_authority`.
...
> Any “small” artifact pushed to every edge (WAF rule, bot features, routing map) has **high blast radius**. AIOps remediation that **pushes config** must go through progressive delivery like product deploys — see [13 — Remediation](../13-remediation-safety/README.md) canary remediation.
...
5. **Observability that does not amplify failure**
   - Rate-limit error enrichment / core dump storms ([14 — Production](../14-production-engine/README.md))
...
> In [13 — Remediation](../13-remediation-safety/README.md), action `restart_pods` needs a **precondition**: not in global overload, dependency healthy, error class = memory leak/single pod — not cascade 503.
...
**Correct correlation** ([10](../10-alert-correlation/README.md)): one incident, root candidate = payment latency.

**Correct RCA** ([11](../11-root-cause-analysis/README.md)): evidence chain of latency edges, not memory OOM.

**Correct remediation** ([13](../13-remediation-safety/README.md)):
...
**Anomaly detection** ([09](../09-anomaly-detection/README.md)): need to suppress secondary anomalies when `protection_mode` is on or when correlation already attached child alerts.
...
- Ingest **migration start/end** events into change correlation ([10](../10-alert-correlation/README.md), [11](../11-root-cause-analysis/README.md)).
...
5. AIOps playbook templates per pattern in knowledge base ([12 — LLM](../12-investigation-engine/README.md))
...
> For each class: mark in architecture review — “do we have early signal? does the action catalog ban dangerous actions? who owns the human skill?” Attach to [14 — Production maturity](../14-production-engine/README.md).
...
#### Detection ([09 — Anomaly Detection](../09-anomaly-detection/README.md))
...
#### Correlation ([10 — Alert Correlation](../10-alert-correlation/README.md))
...
#### RCA ([11 — Root Cause Analysis](../11-root-cause-analysis/README.md))
...
#### LLM ([12 — LLM Agent](../12-investigation-engine/README.md))
...
#### Remediation ([13 — Remediation](../13-remediation-safety/README.md))
...
- Link from AIOps knowledge base for LLM retrieval ([12](../12-investigation-engine/README.md))
...
When [12 — LLM Agent](../12-investigation-engine/README.md) receives an incident embedding near “NXDOMAIN + multi-service,” it must surface the DNS empty card **before** the “restart pods” card.
...
See [14 — Production / Chaos](../14-production-engine/README.md). Famous-incident-derived game days should sit on a **quarterly** calendar, not ad-hoc after outages.
...
| 4 | Synthetics | DNS + HTTPS journey checks ([01](../01-observability/README.md), [09](../09-anomaly-detection/README.md)) |
| 5 | Correlation hubs | Declare dependency hubs in topology ([10](../10-alert-correlation/README.md)) |
| 6 | Remediation gates | Deploy rate-limit + deny mass restart ([13](../13-remediation-safety/README.md)) |
...
2. Which safety gate is missing ([13](../13-remediation-safety/README.md))?
...
2. Which RCA evidence is needed ([11](../11-root-cause-analysis/README.md))?
...
|--------------------|----------|
| Signals & blind spots | [01 Observability](../01-observability/README.md), [09 Anomaly](../09-anomaly-detection/README.md) |
| Event transport resilience | [07 Kafka](../07-kafka/README.md) |
| Hub correlation | [10 Correlation](../10-alert-correlation/README.md) |
| Layered RCA | [11 RCA](../11-root-cause-analysis/README.md) |
| Multi-hypothesis agents | [12 LLM](../12-investigation-engine/README.md) |
| Safety gates | [13 Remediation](../13-remediation-safety/README.md) |
| Chaos & DR & maturity | [14 Production](../14-production-engine/README.md) |
