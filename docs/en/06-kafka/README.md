...
- [07 — Anomaly Detection](../07-anomaly-detection/README.md) — consumes data from Kafka

## Related Documents

- [07 — Anomaly Detection](../07-anomaly-detection/README.md) — consumes telemetry from Kafka, publishes anomaly events
- [08 — Alert Correlation](../08-alert-correlation/README.md) — consumes anomaly events from Kafka
- [11 — Remediation](../11-remediation/README.md) — sends remediation triggers to Kafka
- [15 — Famous Incidents](../15-famous-incidents/README.md) — operational incidents related to transport / cascade failure

## Next Reading

After this chapter, continue to [07 — Anomaly Detection](../07-anomaly-detection/README.md).
...
> [!WARNING]
> **Edge case — key meaning mismatch**: If the correlation engine needs order by `service_name` but the producer keys by `trace_id`, anomalies for the same service land on many partitions → the correlator sees **out-of-order events** → false correlation or missed cascades. Key design is correctness for [08 — Alert Correlation](../08-alert-correlation/README.md), not only throughput.
...
> [!TIP]
> **Why 7 days raw, 30 days anomalies?** Raw telemetry volume is large (metrics/logs/traces) — 7 days is enough for common feature windows (1–7 days) in [07 — Anomaly Detection](../07-anomaly-detection/README.md). Anomaly/alert volume is ~100–1000× smaller → 30 days is cheap and supports post-incident and audit. Incident topics: compact + long retention.
...
| Null service_name | Group-by collapses | Topology correlation fails ([08](../08-alert-correlation/README.md)) |
...
Key = service_name:
  ...
  correlator ([08](../08-alert-correlation/README.md)) collapses 200 events → 1 incident
```
...
|-------|-------------|-------------------|--------|-------|
| `correlation-engine` | seconds | latest (prod) | manual sync | Critical path — see [08](../08-alert-correlation/README.md) |
...
5. Recovery: lag drain + DLQ review; postmortem ([15](../15-famous-incidents/README.md))
...
| 5 | Kafka down, all quiet | MSK network 18 minutes; Prometheus rules already disabled → no page | AIOps as sole path, no bypass | [§24](#24-failure-mode-kafka-down--aiops-blind--bypass), [15](../15-famous-incidents/README.md) |
...
9. Last Kafka-down chaos test? Did Alertmanager bypass fire? ([15](../15-famous-incidents/README.md))
10. Most expensive storage topic: have you computed ingress × retention × RF / compression?
11. [07 — AD](../07-anomaly-detection/README.md) / [08 — Correlation](../08-alert-correlation/README.md): does key design match ordering assumptions?
...
**Next**: [07 — Anomaly Detection](../07-anomaly-detection/README.md) — consume raw from Kafka, publish `aiops-anomalies`.
...
- Downstream: [07 — AD](../07-anomaly-detection/README.md) · [08 — Correlation](../08-alert-correlation/README.md) · [15 — Incidents](../15-famous-incidents/README.md)
