# Chapter 16 — Famous incidents & lessons for the AIOps pipeline
...
- [07 — Anomaly Detection](../08-anomaly-detection/README.md) — early signals and detection limits
- [08 — Alert Correlation](../09-alert-correlation/README.md) — layered alert grouping
- [09 — Root Cause Analysis](../10-root-cause-analysis/README.md) — proximate vs systemic cause
- [11 — Remediation](../12-remediation/README.md) — safety gate, blast radius, rollback
- [12 — Production](../13-production/README.md) — chaos, DR, runbook, maturity
...
After this chapter, return to [12 — Production](../13-production/README.md) to apply design-review checklists and game days to your AIOps platform; or expand your personal incident library per section 11.
...
2. **Audit + change stream for ops tools**
   - Every capacity remove / config push emits an event into [06 — Kafka](../07-kafka/README.md) topic `aiops-change-events`
   - Correlation engine ([08](../09-alert-correlation/README.md)) prioritizes changes in a 15–30 minute window
...
3. **Status / observability independence**
   ...
   - See [01 — Observability](../01-observability/README.md) and [12 — Production](../13-production/README.md)
...
> [!NOTE]
> **AIOps-as-consumer**
> The AIOps platform in this handbook often uses S3-compatible storage for Loki/Tempo/Thanos. Re-read [04 — Loki](../04-loki/README.md), [05 — Tempo](../05-tempo/README.md), [12 — Production](../13-production/README.md) with the question: “If S3-2017 class hits the telemetry bucket, does on-call still have eyes?”
...
3. **Dependency inventory**
   - Catalog “who depends on DynamoDB/regional endpoint” for correlation topology ([09 — RCA](../10-root-cause-analysis/README.md))

4. **Break-glass architecture** ([11 — Remediation](../12-remediation/README.md), [12 — Production](../13-production/README.md))
...
> Rule: multi-phase incidents need a **composite recovery score** (DNS + API + dependency fan-out + backlog). See [08 — Alert Correlation](../09-alert-correlation/README.md) for incident lifecycle states.
...
> [!TIP]
> **AIOps / SRE lesson**
> Dependency inventory must include **physical, identity, badge, DNS, BGP, out-of-band console**. An AIOps topology that only draws the service mesh is **incomplete**. When designing [09 — RCA](../10-root-cause-analysis/README.md), add node types: `network_fabric`, `identity`, `physical_access`, `dns_authority`.
...
> Any “small” artifact pushed to every edge (WAF rule, bot features, routing map) has **high blast radius**. AIOps remediation that **pushes config** must go through progressive delivery like product deploys — see [11 — Remediation](../12-remediation/README.md) canary remediation.
...
5. **Observability that does not amplify failure**
   - Rate-limit error enrichment / core dump storms ([12 — Production](../13-production/README.md))
...
> In [11 — Remediation](../12-remediation/README.md), action `restart_pods` needs a **precondition**: not in global overload, dependency healthy, error class = memory leak/single pod — not cascade 503.
...
**Correct correlation** ([08](../09-alert-correlation/README.md)): one incident, root candidate = payment latency.

**Correct RCA** ([09](../10-root-cause-analysis/README.md)): evidence chain of latency edges, not memory OOM.

**Correct remediation** ([11](../12-remediation/README.md)):
...
**Anomaly detection** ([07](../08-anomaly-detection/README.md)): need to suppress secondary anomalies when `protection_mode` is on or when correlation already attached child alerts.
...
- Ingest **migration start/end** events into change correlation ([08](../09-alert-correlation/README.md), [09](../10-root-cause-analysis/README.md)).
...
5. AIOps playbook templates per pattern in knowledge base ([10 — LLM](../11-llm-agent/README.md))
...
> For each class: mark in architecture review — “do we have early signal? does the action catalog ban dangerous actions? who owns the human skill?” Attach to [12 — Production maturity](../13-production/README.md).
...
#### Detection ([07 — Anomaly Detection](../08-anomaly-detection/README.md))
...
#### Correlation ([08 — Alert Correlation](../09-alert-correlation/README.md))
...
#### RCA ([09 — Root Cause Analysis](../10-root-cause-analysis/README.md))
...
#### LLM ([10 — LLM Agent](../11-llm-agent/README.md))
...
#### Remediation ([11 — Remediation](../12-remediation/README.md))
...
- Link from AIOps knowledge base for LLM retrieval ([10](../11-llm-agent/README.md))
...
When [10 — LLM Agent](../11-llm-agent/README.md) receives an incident embedding near “NXDOMAIN + multi-service,” it must surface the DNS empty card **before** the “restart pods” card.
...
See [12 — Production / Chaos](../13-production/README.md). Famous-incident-derived game days should sit on a **quarterly** calendar, not ad-hoc after outages.
...
| 4 | Synthetics | DNS + HTTPS journey checks ([01](../01-observability/README.md), [07](../08-anomaly-detection/README.md)) |
| 5 | Correlation hubs | Declare dependency hubs in topology ([08](../09-alert-correlation/README.md)) |
| 6 | Remediation gates | Deploy rate-limit + deny mass restart ([11](../12-remediation/README.md)) |
...
2. Which safety gate is missing ([11](../12-remediation/README.md))?
...
2. Which RCA evidence is needed ([09](../10-root-cause-analysis/README.md))?
...
|--------------------|----------|
| Signals & blind spots | [01 Observability](../01-observability/README.md), [07 Anomaly](../08-anomaly-detection/README.md) |
| Event transport resilience | [06 Kafka](../07-kafka/README.md) |
| Hub correlation | [08 Correlation](../09-alert-correlation/README.md) |
| Layered RCA | [09 RCA](../10-root-cause-analysis/README.md) |
| Multi-hypothesis agents | [10 LLM](../11-llm-agent/README.md) |
| Safety gates | [11 Remediation](../12-remediation/README.md) |
| Chaos & DR & maturity | [12 Production](../13-production/README.md) |
