# Chapter 15 — AIOps for E-commerce, Banking & Fintech
...
- [07 — Anomaly Detection](../08-anomaly-detection/README.md) — seasonal baselines, false positives
- [08 — Alert Correlation](../09-alert-correlation/README.md) — topology correlation, alert storms
- [11 — Remediation](../12-remediation/README.md) — safety gate, blast radius, audit
- [12 — Production Operations](../13-production/README.md) — DR, cost, security hardening
...
- [06 — Kafka](../07-kafka/README.md) — cart/payment event pipeline, outbox, exactly-once effect
- [09 — Root Cause Analysis](../10-root-cause-analysis/README.md) — payment rails dependency graph
- [10 — LLM Agent](../11-llm-agent/README.md) — investigation with compliance guardrails
- [13 — BigTech AIOps Patterns](../14-bigtech-aiops/README.md) — scale patterns for reference

## Next Reading

After this chapter, continue to [15 — Famous Incidents](../16-famous-incidents/README.md) to drill public incidents through a money-path lens. You may return to [12 — Production](../13-production/README.md) or [11 — Remediation](../12-remediation/README.md) to tighten the safety matrix.
...
> The model will treat legitimate traffic as anomaly → page on-call → on-call ignores → when a real incident hits during peak, the signal is already dead. See [07 — Anomaly Detection](../08-anomaly-detection/README.md).
...
> At-least-once delivery (Kafka, webhooks, mobile retry) is the default. Exactly-once **business effect** must be designed at the application layer. See [06 — Kafka](../07-kafka/README.md).
...
| Export breaker transitions → bus/metrics | [06 Kafka](../07-kafka/README.md), [08 Correlation](../09-alert-correlation/README.md) |
...
> See the safety framework in [11 — Remediation](../12-remediation/README.md).
...
> Store controlled prompt/response + structured decision fields. See [10 — LLM Agent](../11-llm-agent/README.md).
...
> Use canary failover (5%→25%→50%→100%) + watch secondary saturation. Pattern matches canary remediation in [11 — Remediation](../12-remediation/README.md).
...
Algorithm reference: [07 — Anomaly Detection](../08-anomaly-detection/README.md).
...
Build the engine: [08 — Alert Correlation](../09-alert-correlation/README.md).
...
3. Use temporal order + trace parent links to confirm ([09 — RCA](../10-root-cause-analysis/README.md))
...
From [08](../09-alert-correlation/README.md): dedup pods → group `payment_path_id`/`psp` → topology to PSP → causal order → enrich GMV/campaign/breaker.

**Wrong correlation can be more dangerous than no correlation:** merging fraud + PSP latency into one root then auto-failing over — does not reduce fraud, can burn secondary. Use multi-hypothesis RCA top-k; humans choose when fraud vs infra branches ([09](../10-root-cause-analysis/README.md)).
...
Safety detail: [11 — Remediation](../12-remediation/README.md). LLM propose: [10 — LLM Agent](../11-llm-agent/README.md).
...
Policy table = source of truth; LLM only proposes `action_id` values present in the table ([10](../11-llm-agent/README.md)).
...
DR ops reference: [12 — Production](../13-production/README.md).
...
Platform cost link: [12 — Production](../13-production/README.md).
...
15. AIOps pipeline (Prom/Kafka) dies exactly when PSP is bad — fail-open path ([00](../00-introduction.md), [12](../13-production/README.md)), independent synthetics, platform SLO?
...
| Events, outbox, webhooks | [06 Kafka](../07-kafka/README.md) |
| Seasonal models | [07 Anomaly Detection](../08-anomaly-detection/README.md) |
| Checkout storms | [08 Alert Correlation](../09-alert-correlation/README.md) |
| Dependency RCA | [09 Root Cause Analysis](../10-root-cause-analysis/README.md) |
| Guardrailed investigation | [10 LLM Agent](../11-llm-agent/README.md) |
| Safe auto actions | [11 Remediation](../12-remediation/README.md) |
| DR/cost/security | [12 Production](../13-production/README.md) |
| Scale patterns | [13 BigTech AIOps](../14-bigtech-aiops/README.md) |
...
Apply the checklist in section 14, run game days in sections 10–12, and tightly connect chapters [07](../08-anomaly-detection/README.md)–[11](../12-remediation/README.md) so the intelligence layer truly serves money paths.
