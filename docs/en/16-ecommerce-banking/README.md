...
- [09 — Anomaly Detection](../09-anomaly-detection/README.md) — seasonal baselines, false positives
- [10 — Alert Correlation](../10-alert-correlation/README.md) — topology correlation, alert storms
- [13 — Remediation](../13-remediation-safety/README.md) — safety gate, blast radius, audit
- [14 — Production Operations](../14-production-engine/README.md) — DR, cost, security hardening
...
- [07 — Kafka](../07-kafka/README.md) — cart/payment event pipeline, outbox, exactly-once effect
- [11 — Root Cause Analysis](../11-root-cause-analysis/README.md) — payment rails dependency graph
- [12 — LLM Agent](../12-investigation-engine/README.md) — investigation with compliance guardrails
- [15 — BigTech AIOps Patterns](../15-bigtech-aiops/README.md) — scale patterns for reference

## Next Reading

After this chapter, continue to [17 — Famous Incidents](../17-famous-incidents/README.md) to drill public incidents through a money-path lens. You may return to [14 — Production](../14-production-engine/README.md) or [13 — Remediation](../13-remediation-safety/README.md) to tighten the safety matrix.
...
> The model will treat legitimate traffic as anomaly → page on-call → on-call ignores → when a real incident hits during peak, the signal is already dead. See [09 — Anomaly Detection](../09-anomaly-detection/README.md).
...
> At-least-once delivery (Kafka, webhooks, mobile retry) is the default. Exactly-once **business effect** must be designed at the application layer. See [07 — Kafka](../07-kafka/README.md).
...
| Export breaker transitions → bus/metrics | [07 Kafka](../07-kafka/README.md), [10 Correlation](../10-alert-correlation/README.md) |
...
> See the safety framework in [13 — Remediation](../13-remediation-safety/README.md).
...
> Store controlled prompt/response + structured decision fields. See [12 — LLM Agent](../12-investigation-engine/README.md).
...
> Use canary failover (5%→25%→50%→100%) + watch secondary saturation. Pattern matches canary remediation in [13 — Remediation](../13-remediation-safety/README.md).
...
Algorithm reference: [09 — Anomaly Detection](../09-anomaly-detection/README.md).
...
Build the engine: [10 — Alert Correlation](../10-alert-correlation/README.md).
...
3. Use temporal order + trace parent links to confirm ([11 — RCA](../11-root-cause-analysis/README.md))
...
From [10](../10-alert-correlation/README.md): dedup pods → group `payment_path_id`/`psp` → topology to PSP → causal order → enrich GMV/campaign/breaker.

**Wrong correlation can be more dangerous than no correlation:** merging fraud + PSP latency into one root then auto-failing over — does not reduce fraud, can burn secondary. Use multi-hypothesis RCA top-k; humans choose when fraud vs infra branches ([11](../11-root-cause-analysis/README.md)).
...
Safety detail: [13 — Remediation](../13-remediation-safety/README.md). LLM propose: [12 — LLM Agent](../12-investigation-engine/README.md).
...
Policy table = source of truth; LLM only proposes `action_id` values present in the table ([12](../12-investigation-engine/README.md)).
...
DR ops reference: [14 — Production](../14-production-engine/README.md).
...
Platform cost link: [14 — Production](../14-production-engine/README.md).
...
15. AIOps pipeline (Prom/Kafka) dies exactly when PSP is bad — fail-open path ([00](../00-introduction.md), [14](../14-production-engine/README.md)), independent synthetics, platform SLO?
...
| Events, outbox, webhooks | [07 Kafka](../07-kafka/README.md) |
| Seasonal models | [09 Anomaly Detection](../09-anomaly-detection/README.md) |
| Checkout storms | [10 Alert Correlation](../10-alert-correlation/README.md) |
| Dependency RCA | [11 Root Cause Analysis](../11-root-cause-analysis/README.md) |
| Guardrailed investigation | [12 LLM Agent](../12-investigation-engine/README.md) |
| Safe auto actions | [13 Remediation](../13-remediation-safety/README.md) |
| DR/cost/security | [14 Production](../14-production-engine/README.md) |
| Scale patterns | [15 BigTech AIOps](../15-bigtech-aiops/README.md) |
...
Apply the checklist in section 14, run game days in sections 10–12, and tightly connect chapters [09](../09-anomaly-detection/README.md)–[13](../13-remediation-safety/README.md) so the intelligence layer truly serves money paths.
