...
- [01 — Observability](../01-observability/README.vi.md) — các khái niệm về log, cardinality
- [02 — OpenTelemetry](../02-opentelemetry/README.vi.md) — pipeline thu thập log
- [03 — Prometheus](../03-prometheus/README.vi.md) — mô hình nhãn (Loki sử dụng chung mô hình này)

## Related Documents

- [02 — OpenTelemetry](../02-opentelemetry/README.vi.md) — Loki exporter trong OTel Collector
- [07 — Anomaly Detection](../07-anomaly-detection/README.vi.md) — phát hiện bất thường trên log
- [09 — Root Cause Analysis](../09-root-cause-analysis/README.vi.md) — dữ liệu log làm đầu vào cho RCA

## Next Reading

Sau chương này, hãy chuyển sang [05 — Tempo](../05-tempo/README.vi.md).
...
> [!NOTE]
> **Ý TƯỞNG**
> Loki thắng khi bạn **tôn trọng thiết kế index-labels-only**. Loki thua khi team dùng nó như Elasticsearch (mọi field là index) hoặc khi LogQL trở thành ngôn ngữ chỉ 2 người trong công ty đọc được. Production thinking: *stream cardinality*, *tenant fairness*, *schema migration*.

### 18.1 Vì sao chỉ index labels — đây là trade-off có chủ đích

Loki lấy cảm hứng Prometheus:

- **Labels** → xác định *stream* (index nhỏ, rẻ, RAM-sensitive).
- **Nội dung log line** → lưu chunk (object storage), filter lúc query.

| Nếu bạn... | Hệ quả |
|------------|--------|
| Thêm `user_id` vào label | Stream nổ → ingester RAM chết |
| Để `user_id` trong JSON line | Query chậm hơn chút nhưng scale được |
| Query không có stream selector | Quét rộng → timeout / công bằng tenant vỡ |
| Có quá ít labels | Khó thu hẹp → cũng chậm |

> [!TIP]
> **Vì sao**
> Elasticsearch index full-text mua **linh hoạt query** bằng **chi phí RAM/cluster**. Loki mua **$/GB rẻ** bằng **kỷ luật label + thói quen LogQL**. AIOps team phải thiết kế schema log *trước*, không "index tất cả rồi tính sau".

### 18.2 LogQL cognitive load — ngôn ngữ mạnh, dễ viết query tự hại

LogQL kết hợp:

1. **Stream selector** `{app="api"}` — bắt buộc tư duy index.
2. **Line filters** `|=`, `!=`, `|~` — rẻ hơn parser nếu đủ.
3. **Parsers** `| json`, `| logfmt`, `| regexp` — CPU query path.
4. **Label filters** sau parse `| level="error"` — trên kết quả parse.
5. **Metric queries** `rate`, `topk` — biến log thành time series.

Tư duy giảm tải nhận thức:

```text
Viết LogQL như pipeline tối ưu:
  hẹp stream → lọc line rẻ → parse → filter field → aggregate
Không:
  parse tất cả 7 ngày rồi filter app (nhầm thứ tự / thiếu selector)
```

> [!WARNING]
> **Edge**
> Query "tìm string toàn cluster 24h" có thể **DOSS chính Loki** và hàng xóm tenant. Cần limits, timeout, và văn hóa query có selector.

### 18.3 Noisy neighbor tenants — đa thuê là dao hai lưỡi

Multi-tenant tách bằng `X-Scope-OrgID` (hoặc auth gateway). Noisy neighbor:

- Tenant A stream cardinality cao → ingester shared pressure.
- Tenant B query quét khủng → querier/scheduler bão.
- Tenant C retention/volume billing mù → cost surprise.
...
Chuyển từ plain text → JSON không chỉ là "đổi format":

1. **Dual-write / dual-parse** giai chuyển (cũ và mới cùng sống).
2. **Field name drift** (`msg` vs `message`, `ts` vs `time`).
3. **Dashboard & alert LogQL** gãy hàng loạt.
4. **Collector transform** phải normalize trước Loki.
5. **App libraries** không đồng nhất (Java/Python/Node).

> [!NOTE]
> **Ý TƯỞNG**
> Migration thành công khi có **schema contract** + **enforce tại collector** + **canary service**, không khi "gửi email nhờ anh em log JSON giùm".

### 18.5 Vòng problem-solving Loki
...
Symptom split:
  - mất log mới? → ingest/limits/auth tenant
  - query chậm? → selector/cardinality/cache/compactor
  - cost cao? → volume + retention + churn streams
  - sai log? → label design / parser / multi-tenant mix
```
...
### EC-01 — Label `request_id` trên stream

| | |
|--|--|
| **Triệu chứng** | Ingester OOM; `memory_streams` tăng thẳng đứng. |
| **Nguyên nhân** | Mỗi request một stream. |
| **Phát hiện** | Stream count per tenant; top label values. |
| **Phòng** | Label policy; reject high-card labels tại gateway/alloy. |

### EC-02 — Query không stream selector

| | |
|--|--|
| **Triệu chứng** | UI timeout; querier CPU 100%. |
| **Nguyên nhân** | `{job=~".+"} |= "error"` trên quá rộng. |
| **Phát hiện** | Query frontend logs; slow query log. |
| **Phòng** | Enforce required labels; max query series/timeout; training. |

### EC-03 — json parse trên log không phải JSON

| | |
|--|--|
| **Triệu chứng** | Metric query từ log = 0 oan; dashboard trống. |
| **Nguyên nhân** | 30% line plain text; `| json` fail silent trong pipeline. |
| **Phát hiện** | Sample lines; `| json | __error__!=""` patterns. |
| **Phòng** | Normalize tại collector; stage migration flags. |

### EC-04 — Noisy tenant query storm

| | |
|--|--|
| **Triệu chứng** | Tenant khác query chậm dù volume nhỏ. |
| **Nguyên nhân** | Shared queriers; không fair scheduling limits. |
| **Phát hiện** | Per-tenant query metrics; scheduler queue. |
| **Phòng** | Per-tenant max concurrency; separate priority; rate limit. |

### EC-05 — Out-of-order / timestamp skew drops

| | |
|--|--|
| **Triệu chứng** | Thiếu log quanh NTP issues / batch late. |
| **Nguyên nhân** | Ingester reject out-of-order vượt cửa sổ. |
| **Phát hiện** | Discard metrics; agent clock. |
| **Phòng** | clock sync; configure acceptable window; prefer agent timestamp policy rõ. |

### EC-06 — Structured migration: field rename gãy alert

| | |
|--|--|
| **Triệu chứng** | Alert `level="ERROR"` im; production đang lỗi. |
| **Nguyên nhân** | App đổi `level` → `severity`. |
| **Phát hiện** | Compare parse success ratio; canary alerts. |
| **Phòng** | Schema version field; collector map aliases; contract tests. |

### EC-07 — Cardinality từ `pod` + `path` + `status` ...

| | |
|--|--|
| **Triệu chứng** | Stream limit hit; ingest 429. |
| **Nguyên nhân** | Quá nhiều label dims dù từng cái "có vẻ bounded". |
| **Phát hiện** | Stream growth after deploy. |
| **Phòng** | Budget combinations; drop pod from labels if app label đủ. |

### EC-08 — Cache miss storm sau deploy querier

| | |
|--|--|
| **Triệu chứng** | S3 GET nổ; bill tăng; latency query. |
| **Nguyên nhân** | Cold cache; không memcached; parallel heavy. |
| **Phát hiện** | Cache hit ratio; S3 metrics. |
| **Phòng** | Chunk/index cache; warm critical dashboards; limit fanout. |

### EC-09 — Compactor/retention xóa nhầm kỳ vọng pháp lý

| | |
|--|--|
| **Triệu chứng** | Audit log biến mất trước hạn. |
| **Nguyên nhân** | Global retention; tenant audit không tách. |
| **Phát hiện** | Retention config vs compliance matrix. |
| **Phòng** | Tenant retention overrides; separate stream for audit. |

### EC-10 — Multiline stacktrace bị vỡ dòng

| | |
|--|--|
| **Triệu chứng** | Java exception không search được full. |
| **Nguyên nhân** | Agent split lines; không multiline config. |
| **Phát hiện** | Visual sample; compare app file vs Loki. |
| **Phòng** | Multiline stage Alloy/Promtail; JSON log 1-line exceptions. |

### EC-11 — Ruler LogQL metric spam eval

| | |
|--|--|
| **Triệu chứng** | Ruler CPU cao; alert trễ. |
| **Nguyên nhân** | Rules quét rộng interval ngắn. |
| **Phát hiện** | Ruler metrics; rule group duration. |
| **Phòng** | Recording-like narrow selectors; interval hợp lý; split groups. |

### EC-12 — Auth gateway default tenant

| | |
|--|--|
| **Triệu chứng** | Logs "mất" — thực ra vào tenant `fake`/default. |
| **Nguyên nhân** | Thiếu OrgID; multi-tenant mode on. |
| **Phát hiện** | Query đúng tenant; distributor labels. |
| **Phòng** | Reject missing tenant; dashboards có tenant var; integration tests. |

---

## 20. Decision trees

### 20.1 Field mới: label hay nội dung log?

```mermaid
flowchart TD
    A[Field mới cần quan sát] --> B{Bounded low cardinality?}
    B -->|Không| C[JSON body / structured line]
    B -->|Có| D{Dùng để thu hẹp hầu hết query?}
    D -->|Có| E[Cân nhắc label — kiểm budget tổ hợp]
    D -->|Không| C
    E --> F{Tổ hợp labels vượt budget?}
    F -->|Có| G[Bỏ label ít giá trị nhất → body]
    F -->|Không| H[Add label + monitor streams]
```
...
```mermaid
flowchart TD
    A[LogQL timeout] --> B{Có stream selector hẹp?}
    B -->|Không| C[Thêm app/env/namespace]
    B -->|Có| D{Time range quá rộng?}
    D -->|Có| E[Thu hẹp + dùng metric query nếu cần]
    D -->|Không| F{Parse nặng trên volume lớn?}
    F -->|Có| G[Line filter trước parse; chuẩn hóa schema]
    F -->|Không| H[Cache / querier scale / hot streams?]
```
...
```mermaid
flowchart TD
    A[Tenant khác bị chậm] --> B{Ingest hay query path?}
    B -->|Ingest| C[Streams/rate limit tenant A]
    B -->|Query| D[Concurrency limit + timeout tenant A]
    C --> E[Liên hệ owner + cardinality fix]
    D --> E
    E --> F{Tách cluster/dedicated?}
    F -->|Enterprise large| G[Shard tenants / cell architecture]
    F -->|Chưa| H[Giữ shared + hard limits]
```

### 20.4 Migration structured logging

```text
1. Chốt schema (field names, levels, trace_id)
2. Collector normalize (cả legacy)
3. Canary 1 service → dashboards/alerts
4. Dual support N weeks
5. Enforce drop non-compliant (hoặc quarantine tenant)
...
Nhiều org công bố logging là **một trong những bill cloud lớn**. Chuyển ELK → Loki/tương đương thường vì **economics**, nhưng fail nếu không đổi văn hóa label/query.

**Bài học**: cost dashboards per tenant/service; sampling debug logs; retention tiering.

### 21.2 Multi-tenant fairness

SaaS observability vendors build hard isolation vì **một customer** có thể query-of-death. Self-host Loki multi-team = mini-SaaS — cần limits giống vendor.

### 21.3 Schema without governance

Postmortems nội bộ hay thấy: sau 1 năm, 5 tên field cho "message" → MTTR tăng. Platform thắng nhờ **central transform** hơn là tin developer discipline thuần.

### 21.4 Index everything hangover

Team từ ELK mang thói quen index mọi field → stream explosion. Bài học change management: training + linter + reject.

### 21.5 Map Ch13 / Ch15

| Bài học | Loki chapter | Sang |
|---------|--------------|------|
...
### 22.1 Mất log / thiếu log

1. Tenant ID bạn query có đúng với ingest không?
2. Mất ở tail (mới) hay history (cũ) — path nào?
3. Agent có ship không? Distributor từ chối vì limit?
4. Label set có đổi sau deploy (stream mới) không?
5. Out-of-order discard có tăng không?

### 22.2 Query & LogQL

6. Selector có hẹp nhất có thể không?
7. Bạn parse trước khi filter line rẻ — có cần không?
8. Time range có lớn hơn nhu cầu IR không?
9. Query này có thể giết hàng xóm không?
10. Có slow query log / example tối ưu không?

### 22.3 Cardinality & tenants

11. Stream count tenant có trong budget không?
12. Label nào tăng sau release?
13. Limits đang punish innocent tenants chứ?
14. Ruler rules có selector an toàn không?
15. Compliance retention có bị global policy đè không?

### 22.4 Sau sự cố / migration

16. Schema field có contract test không?
17. Collector đã normalize alias chưa?
18. Cần thêm cache hay bớt query thói quen?
19. Experiment 30 ngày giảm volume DEBUG?
20. Log đã đủ structured cho correlation TraceID (Ch01) chưa?

---

## 23. Improvement experiments (30/60/90 ngày)

### 30 ngày — Discipline foundation

| Experiment | Cách làm | Success metric |
|------------|---------|----------------|
| Label policy | Allowlist + examples | 0 high-card labels mới |
| Stream inventory | Top tenants by streams | Owners identified |
| Query education | Workshop + cheat sheet | Slow queries ↓ 30% |
| Tenant limits | Rate + concurrency | Noisy incident giảm |
| TraceID field audit | % logs có trace_id | Baseline số |

**Deliverables**: label policy; tenant limit values documented.

### 60 ngày — Schema & performance
...
| Dedicated path audit logs | Separate retention | Compliance OK |
...
> [!TIP]
> **Vì sao**
> Loki trả giá rẻ chỉ khi org **trả bằng kỷ luật**. 30/60/90 xây kỷ luật đó thành thói quen đo được — không phải slide architecture.
