# Chapter 14 — AIOps cho E-commerce, Banking & Fintech
...
- [07 — Anomaly Detection](../07-anomaly-detection/README.vi.md) — baseline theo mùa, false positive
- [08 — Alert Correlation](../08-alert-correlation/README.vi.md) — topology correlation, alert storm
- [11 — Remediation](../11-remediation/README.vi.md) — safety gate, blast radius, audit
- [12 — Production Operations](../12-production/README.vi.md) — DR, cost, security hardening
...
- [06 — Kafka](../06-kafka/README.vi.md) — event pipeline cart/payment, outbox, exactly-once effect
- [09 — Root Cause Analysis](../09-root-cause-analysis/README.vi.md) — dependency graph payment rails
- [10 — LLM Agent](../10-llm-agent/README.vi.md) — investigation có guardrail compliance
- [13 — BigTech AIOps Patterns](../13-bigtech-aiops/README.vi.md) — scale patterns tham chiếu

## Next Reading

Sau chương này, chuyển sang [15 — Famous Incidents](../15-famous-incidents/README.vi.md) để drill các sự cố công khai với góc nhìn money-path. Có thể quay lại [12 — Production](../12-production/README.vi.md) hoặc [11 — Remediation](../11-remediation/README.vi.md) để siết safety matrix.
...
> Model sẽ coi traffic hợp lệ là anomaly → page on-call → on-call ignore → khi sự cố thật xảy ra trong peak, signal đã chết. Xem [07 — Anomaly Detection](../07-anomaly-detection/README.vi.md).
...
> At-least-once delivery (Kafka, webhooks, mobile retry) là mặc định. Exactly-once **business effect** phải được thiết kế ở application layer. Xem [06 — Kafka](../06-kafka/README.vi.md).
...
| Export breaker transitions → bus/metrics | [06 Kafka](../06-kafka/README.vi.md), [08 Correlation](../08-alert-correlation/README.vi.md) |
...
> Xem safety framework ở [11 — Remediation](../11-remediation/README.vi.md).
...
> Lưu prompt/response có kiểm soát + decision structured fields. Xem [10 — LLM Agent](../10-llm-agent/README.vi.md).
...
> Dùng canary failover (5%→25%→50%→100%) + theo dõi secondary saturation. Pattern giống canary remediation ở [11 — Remediation](../11-remediation/README.vi.md).
...
Tham chiếu thuật toán: [07 — Anomaly Detection](../07-anomaly-detection/README.vi.md).
...
Xây engine: [08 — Alert Correlation](../08-alert-correlation/README.vi.md).
...
3. Dùng temporal order + trace parent links để xác nhận ([09 — RCA](../09-root-cause-analysis/README.vi.md))
...
Từ [08](../08-alert-correlation/README.vi.md): dedup pods → group `payment_path_id`/`psp` → topology tới PSP → causal order → enrich GMV/campaign/breaker.

**Correlation sai có thể nguy hiểm hơn không correlation:** gộp fraud + PSP latency thành một root rồi auto failover — không giảm fraud, có thể đốt secondary. Dùng multi-hypothesis RCA top-k; human chọn khi nhánh fraud vs infra ([09](../09-root-cause-analysis/README.vi.md)).
...
Chi tiết safety: [11 — Remediation](../11-remediation/README.vi.md). LLM propose: [10 — LLM Agent](../10-llm-agent/README.vi.md).
...
Policy table = source of truth; LLM chỉ propose `action_id` có trong bảng ([10](../10-llm-agent/README.vi.md)).
...
Tham chiếu DR ops: [12 — Production](../12-production/README.vi.md).
...
Liên hệ cost platform: [12 — Production](../12-production/README.vi.md).
...
15. AIOps pipeline (Prom/Kafka) hỏng đúng lúc PSP bad — fail-open path ([00](../00-introduction.vi.md), [12](../12-production/README.vi.md)), synthetic độc lập, platform SLO?
...
| Events, outbox, webhooks | [06 Kafka](../06-kafka/README.vi.md) |
| Seasonal models | [07 Anomaly Detection](../07-anomaly-detection/README.vi.md) |
| Checkout storms | [08 Alert Correlation](../08-alert-correlation/README.vi.md) |
| Dependency RCA | [09 Root Cause Analysis](../09-root-cause-analysis/README.vi.md) |
| Guardrailed investigation | [10 LLM Agent](../10-llm-agent/README.vi.md) |
| Safe auto actions | [11 Remediation](../11-remediation/README.vi.md) |
| DR/cost/security | [12 Production](../12-production/README.vi.md) |
| Scale patterns | [13 BigTech AIOps](../13-bigtech-aiops/README.vi.md) |
...
Áp dụng checklist mục 14, chạy game days mục 10–12, và nối chặt với các chương [07](../07-anomaly-detection/README.vi.md)–[11](../11-remediation/README.vi.md) để intelligence layer thực sự phục vụ money paths.
